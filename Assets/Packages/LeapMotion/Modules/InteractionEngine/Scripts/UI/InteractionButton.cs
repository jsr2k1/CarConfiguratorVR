  ...
  public class InteractionButton : InteractionBehaviour {
    ...
    protected virtual void onGraspEnd() {
      if (localPhysicsPosition.z > initialLocalPosition.z - minMaxHeight.x) {
        ...
        protected virtual void OnCollisionEnter(Collision collision) { PosibleClick(collision.collider); trySetDepressor(collision.collider); }
        protected virtual void OnCollisionStay(Collision collision) { trySetDepressor(collision.collider); }

//joel  float TimeAwake = 0;
...
//          TimeAwake = Time.time;
        ...
        private void PosibleClick(Collider collider)
        {
            return;
//joel        if (!isIndex(collider)) return;
//
...
//            {                
//                if (Time.time - TimeAwake > 0.5)
//                {
//                    OnPress.Invoke();
//                }                
//            }
        }
        private bool isIndex(Collider collider)
        {
            if (collider.attachedRigidbody == null || !manager.contactBoneBodies.ContainsKey(collider.attachedRigidbody) ) return false;
            Utils.ChildrenEnumerator child = manager.contactBoneBodies[collider.attachedRigidbody].transform.parent.GetChildren();
            return (child.MoveNext() && child.MoveNext() && child.MoveNext() && child.MoveNext() && child.MoveNext() && child.MoveNext() && child.Current.gameObject == collider.gameObject); //Modo test
        }
