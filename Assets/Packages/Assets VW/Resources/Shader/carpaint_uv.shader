...
Shader "VW Shaders/Carpaint UV-Unity5"
{
	Properties
	{
		...
		_MicroflakeSpecular ("Flake Specular", Float) = 0.75
		_MicroflakePerturbation ("Flake Weight", Float) = 1
		_NormalPerturbation ("Normal Weight", Float) = 0.5
...
//	Cull Back 
	...
	float4 frag(VertexOutput i) : COLOR
	{
		...
		float3 k6 = _LightColor0.rgb; float3 l6 = normalize(l7+k1);
		half l8 = LIGHT_ATTENUATION(i); half3 l9 = l8 * _LightColor0.xyz; 
		UnityLight light; 
	...
	#endif
	k1 = gi.light.dir; k6 = gi.light.color; half3 i1; 
	#ifdef FLAKES_ON
	half k4 = length(_WorldSpaceCameraPos.xyz - i.posWorld.xyz); half3 k8 = UnpackNormal(tex2D(_FlakesTex,TRANSFORM_TEX(inputUV.rg, _FlakesTex))).rgb; 
	#endif
